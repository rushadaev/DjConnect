	...
	<div class="flex flex-col w-full gap-1">
		...
		<div class="relative flex justify-center items-center">
			<input
				...
				inputmode="numeric"
				class="bg-transparent text-5xl focus:border-greenYellow block w-full rounded-md p-3 text-[#FFFFFF4D]"
				:class="{
					'border-grey border-1 border-solid':
						props?.priceChangeAllowed,
					'border-0': !props?.priceChangeAllowed
				}"
				:disabled="disabled"
				@blur="formatPrice"
				@input="updateValue"
...
<script setup lang="ts">
	...
	const updateValue = (event: Event) => {
		let value = (event.target as HTMLInputElement).value.replace(
			/[^\d]/g,
			''
		)

<template>
	<div class="px-6 relative z-10 pt-[20px] overflow-x-hidden">
		<h1 class="text-2xl pb-4 font-black">
			Список заказов
		</h1>
...
<script setup lang="ts">
	...
	const updateOrder = (orderUpdate: any) => {
		...
		const { statusText, statusColor } = getStatusText(
			orderUpdate.status,
			orderUpdate.is_paid
		)
		if (index !== -1) {
	...
	onMounted(async () => {
		if (user.value?.is_dj && flow !== 'user') {
			...
			if (user.value.dj) subscribeToOrderCreate(user.value.dj.id)
		} else {

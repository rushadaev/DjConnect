...
const mockTWA: TWA = {
    ...
    offEvent: (eventName: string, eventHandler: (...args: any[]) => unknown) => {
    ...
    CloudStorage: {
        ...
        removeItems: (keys: string[]): Promise<void> => {
            console.log(`Removing multiple items from CloudStorage: ${keys.join(', ')}`)
            return Promise.resolve()
        },
        getItems: (keys: string[]): Promise<(string | null)[]> => {
            console.log(`Getting multiple items from CloudStorage: ${keys.join(', ')}`)
    ...
    showScanQrPopup: (params: { text?: string }, callback?: (text: string) => void) => {

...
import { DJ, DJRegistration, Track } from './types'
import { useSessionStore } from '@/entities/session/model/session.store'
import type { Order, Statistics } from '@/features/order-music/model'
import type {
	PayoutRequest,
	PayoutStatus
} from '@/features/edit-card/model/types'
...
export const useDJStore = defineStore('dj', {
	...
	actions: {
		...
		async orderTrackRequest(price: number, message: string) {
			...
			this.error = null
			try {
				const { error: apiError, execute } = useApi<void>(
					'post',
					'/orders',
					{
		...
		async registerDJ(
			...
			try {
				const {
					...
					error: apiError,
					execute
				} = useApi<DJ>('post', '/dj/register', djData)
				await execute()
				if (apiError.value) throw new Error(apiError.value)
				if (data.value) {
		...
		async updateDJProfile(djData: Partial<DJ> | FormData) {
			this.isLoading = true
			this.error = null
			...
			try {
				const {
					...
					error: apiError,
					execute
				} = useApi<DJ>('post', `/dj/profile/${djId}`, djData)
				await execute()
				if (apiError.value) throw new Error(apiError.value)
				if (data.value) {
					this.currentDJ = data.value
		...
		async fetchDJStatistics(djId: number): Promise<Statistics> {
			...
			try {
				const {
					...
					error: apiError,
					execute
				} = useApi<Statistics>('get', `/dj/${djId}/statistics`)
				await execute()
				if (apiError.value) throw new Error(apiError.value)
		...
		async fetchDJOrders(djId: number): Promise<Order[]> {
			...
			try {
				const {
					...
					error: apiError,
					execute
				} = useApi<Order[]>('get', `/dj/${djId}/orders`)
				await execute()
				if (apiError.value) throw new Error(apiError.value)
		...
		async fetchOrder(orderId: number): Promise<Order> {
			...
			try {
				const {
					...
					error: apiError,
					execute
				} = useApi<Order>('get', `/orders/${orderId}`)
				await execute()
				if (apiError.value) throw new Error(apiError.value)
				return data.value as Order
			} catch (error) {
		...
		async pollOrderStatus(orderId: number): Promise<Order> {
			...
			try {
				const {
					...
					error: apiError,
					execute
				} = useApi<Order>('get', `/orders/${orderId}/status`)
				await execute()
				if (apiError.value) throw new Error(apiError.value)
				return data.value as Order
			} catch (error) {
		...
		async fetchTracks(djId: number) {
			...
			try {
				const {
					...
					error: apiError,
					execute
				} = useApi<Track[]>('get', `/dj/${djId}/tracks`)
				await execute()
				if (apiError.value) throw new Error(apiError.value)
				this.tracks = data.value || []
				console.log('this.tracks', this.tracks)
				if (this.currentDJ) this.currentDJ.tracks = this.tracks ?? []
		...
		async addTrack(djId: number, name: string) {
			...
			try {
				const {
					...
					error: apiError,
					execute
				} = useApi<Track>('post', `/dj/${djId}/track`, { name })
				await execute()
				if (apiError.value) throw new Error(apiError.value)
				if (data.value) {
		...
		async updateTrackPrice(trackId: number, price: number) {
			...
			try {
				const {
					...
					'patch',
					`/dj/${this.currentDJ?.id}/track/${trackId}/price`,
					{ price }
				)
				await execute()
				if (apiError.value) throw new Error(apiError.value)
		...
		async createPayoutRequest(
			...
			try {
				const {
					...
					error: apiError,
					execute
				} = useApi<PayoutStatus>('post', '/payouts', {
					dj_id: djId,
					amount: amount,
